...
function FollowingLine({ d, progress, index, still, extended }) {
  ...
  const drawn = useTransform(
    progress,
    extended ? [0.04, 0.8] : [0.005 + index * 0.015, 0.15 + index * 0.015],
    [0, 1],
  );
  ...
  const sync = (p) => {
    ...
    const point = path.current.getPointAtLength(p * length.current);
    cx.set(point.x);
    cy.set(point.y);
  };
  ...
  return (
    <g className={`scroll-ribbon scroll-ribbon-${index}`}>
      <path
        d={d}
      ...
      <motion.path
        ref={path}
        d={d}
        style={still ? { pathLength: 1 } : { pathLength: drawn }}
      />
      {!still && (
        <motion.circle
          r="6"
          style={{ cx, cy, opacity: drawn }}
        />
      )}
    </g>
  );
...
export function ScrollRibbons({ progress, paused = false, extended = false }) {
  ...
  return (
    <svg
      className="scroll-ribbons"
      viewBox="0 0 1440 900"
      preserveAspectRatio="none"
      aria-hidden="true"
    >
      <FollowingLine
        d="M -60 130 C 300 210 440 240 735 125 S 1230 -70 1300 260 S 1310 620 1500 635"
        progress={progress}
        index={0}
        extended={extended}
        still={paused || reduced}
      />
      <FollowingLine
        d="M -60 335 C 260 440 520 470 825 325 S 1230 120 1275 420 S 1330 690 1500 620"
        progress={progress}
        index={1}
        extended={extended}
        still={paused || reduced}
