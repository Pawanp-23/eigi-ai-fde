...
function FollowingLine({d,progress,index,still}){
 const path=useRef(null),length=useRef(0),cx=useMotionValue(0),cy=useMotionValue(0);
 const drawn=useTransform(progress,[.005+index*.015,.15+index*.015],[0,1]);
...
export function ScrollRibbons({progress,paused=false}){
 const reduced=useReducedMotion();
 return <svg className="scroll-ribbons" viewBox="0 0 1440 900" preserveAspectRatio="none" aria-hidden="true">
  <FollowingLine d="M -60 130 C 300 210 440 240 735 125 S 1230 -70 1300 260 S 1310 620 1500 635" progress={progress} index={0} still={paused||reduced}/>
  <FollowingLine d="M -60 335 C 260 440 520 470 825 325 S 1230 120 1275 420 S 1330 690 1500 620" progress={progress} index={1} still={paused||reduced}/>
 </svg>;
}
