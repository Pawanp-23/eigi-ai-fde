...
import { MotionConfig, motion, useScroll, useMotionValue, useMotionValueEvent, useReducedMotion } from 'motion/react';
...
export default function Expedition({paused,contact}){
 ...
 function go(progress){const el=section.current;if(!el)return;window.scrollTo({top:el.getBoundingClientRect().top+window.scrollY+progress*(el.offsetHeight-window.innerHeight),behavior:'instant'});}
 return <MotionConfig reducedMotion="user"><section ref={section} id="experience" className={`expedition ${reduced?'expedition-reduced':''} ${failed?'expedition-failed':''}`} aria-label="A journey from possibility to production">
  <div className={`expedition-stage ${chapter>0&&!reduced?'expedition-dark':''}`}>
   ...
   <motion.div className="expedition-editorial" style={reduced||failed?{}:{opacity:introOpacity,y:introY}} aria-hidden={chapter!==0&&!reduced}>
    <h2>Ideas deserve<br/>another dimension.</h2>
    <div className="expedition-editorial-copy"><p>Big ideas need engineers who can take them somewhere real.</p><p>We step into your world, connect the pieces, and build systems that take you further. From the first question to the next breakthrough.</p><span>SCROLL TO STEP INSIDE <ArrowDown size={16}/></span></div>
   </motion.div>
   {!reduced&&!failed&&<>
    ...
    <div className="expedition-progress" aria-hidden="true"><motion.span style={{scaleX:scrollYProgress}}/></div>
   </>}
