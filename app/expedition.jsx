...
export default function Expedition({paused,contact,engineeringCopy=false}){
 ...
 return <MotionConfig reducedMotion="user"><section ref={section} id="experience" className={`expedition ${reduced?'expedition-reduced':''} ${failed?'expedition-failed':''}`} aria-label="A journey from possibility to production">
  <div className={`expedition-stage ${chapter>0&&!reduced?'expedition-dark':''}`}>
   ...
   <motion.div className="expedition-editorial" style={reduced||failed?{}:{opacity:introOpacity,y:introY}} aria-hidden={chapter!==0&&!reduced}>
    <h2><ScrollWords text={engineeringCopy?"Give your big idea":"Ideas deserve"} paused={paused}/><br/><ScrollWords text={engineeringCopy?"the website it deserves.":"another dimension."} paused={paused}/></h2>
   ...
   {!reduced&&!failed&&<>
    ...
    <motion.div className="expedition-finale expedition-overlay" style={{opacity:finaleOpacity,y:finaleY,pointerEvents:chapter===3?'auto':'none'}} aria-hidden={chapter!==3}><span className="finale-hello">Hello. Ready for what's next?</span><h2>Let’s build<br/>what’s next.</h2><a href={contact} className="expedition-cta" tabIndex={chapter===3?0:-1}>LET’S TALK <ArrowUpRight size={20}/></a></motion.div>
