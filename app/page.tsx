...
import { ArrowUpRight, ArrowDown, Plus, Pause, Play, X, MoveUpRight, Sun, Moon } from 'lucide-react';
...
export default function Home(){
 ...
 useEffect(()=>{
  ...
  const sync=()=>{let saved=null;try{saved=localStorage.getItem('eigi-theme');}catch{}const value=saved?saved==='dark':media.matches;setDark(value);document.documentElement.dataset.theme=value?'dark':'light';document.documentElement.classList.toggle('dark',value);};
  sync();media.addEventListener('change',sync);return()=>media.removeEventListener('change',sync);
 },[]);
 const toggleTheme=()=>{const value=!dark;setDark(value);document.documentElement.dataset.theme=value?'dark':'light';document.documentElement.classList.toggle('dark',value);try{localStorage.setItem('eigi-theme',value?'dark':'light');}catch{}};
 useEffect(()=>{const query=new URLSearchParams(window.location.search);setSignalHero(!query.has('hero')&&query.get('preview')!=='hero');},[]);
 useEffect(()=>{const query=new URLSearchParams(window.location.search);setPreview(query.get('preview')==='hero');setVideoHero(query.get('hero')!=='original');setNewHero(query.get('preview')==='hero'&&query.get('hero')!=='original');},[]);
 ...
 return <main id="top" className={signalHero?"signal-page":newHero?"engineering-preview":videoHero?"fullscreen-film-page":""}><div className="page-progress" aria-hidden="true"/>
  <ParallaxComponent paused={paused}/>
  <a href="#approach" className="skip-link">Skip to content</a>
  {newHero?<EngineeringHero contact={contact} paused={paused} onPause={()=>setPaused(!paused)} onMenu={()=>setMenu(true)}/>:<>
  <header className="site-header frosted-header">
   <a href="#top" className="wordmark brand-lockup" aria-label="eigi ai home"><img src="/eigi-logo.ico" alt="" width="32" height="32"/>eigi.ai</a>
   ...
   <nav className="header-actions" aria-label="Main navigation">
    <div className="header-section-links"><a href="#method">Our FDE</a><a href="#approach">FDE Goals</a><a href="#contact">Contact Us</a><a href="#opportunities">Opportunities</a></div>
    <button className="theme-control" onClick={toggleTheme} aria-label={dark?'Switch to light mode':'Switch to dark mode'} title={dark?'Switch to light mode':'Switch to dark mode'}>{dark?<Sun size={19}/>:<Moon size={19}/>}</button>
  ...
  {signalHero?<SignalHero paused={paused} contact={contact}/>:<section className={`scroll-story ${videoHero?'video-story':''}`} aria-label="From complexity to production">
   <div className={`hero-scene story-sticky ${assembled?'assembled':''}`}>
    {videoHero?<HeroVideo paused={paused}/>:<Frontier assembled={assembled} paused={paused} motion={motion}/>}
    <div className="hero-shade"/>
    <p className="scene-eyebrow"><span className="status-dot"/> FORWARD DEPLOYED ENGINEERING</p>
    <HeroPunchline/>
    <div className="hero-bottom"><a href="#experience" className="story-skip">ENTER THE 3D EXPERIENCE <ArrowUpRight size={16}/></a>{videoHero?<span className="film-label">HUMAN INGENUITY. REAL-WORLD IMPACT.</span>:<button className="assemble-button" onClick={()=>setAssembled(!assembled)} aria-pressed={assembled}>{assembled?'RELEASE THE SIGNAL':'FOCUS THE SIGNAL'}<Plus size={18} className={assembled?'rotated':''}/></button>}</div>
  ...
  {preview&&<nav className="hero-comparison" aria-label="Hero comparison"><span>LOCAL PREVIEW</span><a href="/?preview=hero" aria-current={newHero?"page":undefined}>New hero</a><a href="/?preview=hero&hero=original" aria-current={!newHero?"page":undefined}>Previous hero</a></nav>}
  ...
  <section id="approach" className="intro section-pad">
   <div className="intro-top" data-reveal><p className="eyebrow">01 / THE EIGI APPROACH</p><span className="little-mark">eigi.ai</span></div>
   <h2 data-reveal data-scroll-shift="-30">{newHero?<><KineticText text="The space between"/><br/><KineticText text="ambition and execution."/></>:<><KineticText text="Great AI belongs"/><br/>in the <span className="blue-text"><KineticText text="real world."/></span><span className="heading-dot">✳</span></>}</h2>
  ...
  <section id="method" className="method section-pad">
   <div className="method-heading" data-reveal><p className="eyebrow">02 / FORWARD, TOGETHER</p><h2>{newHero?<><KineticText text="Turn your vision into"/><br/><KineticText text="reality that runs"/><br/><KineticText text="your business."/></>:<><KineticText text="Your team."/><br/><KineticText text="Extended."/></>}</h2><p>From the first hard question to the system your team uses every day. We work alongside you through the whole journey.</p><a className="pill light" href={contact}>BUILD WITH US <ArrowUpRight size={17}/></a></div>
  ...
  <ScrollMorphHero paused={paused} contact={contact}/>
  <EigiTeam paused={paused}/>
  <section id="opportunities" className="opportunities section-pad"><div><p className="eyebrow">OPPORTUNITIES / BUILD WITH EIGI</p><h2>Make your next<br/>chapter matter.</h2></div><div><p>Interested in building useful AI with us? Tell us about your work, your interests, and where you could contribute.</p><a className="pill dark" href="mailto:pawanpatil2305@gmail.com?subject=Opportunities%20at%20Eigi.ai">START A CONVERSATION <ArrowUpRight size={18}/></a></div></section>
  <footer id="contact" className="footer"><div className="footer-top"><a href="#top" className="wordmark footer-brand" aria-label="Eigi AI home"><img src="/eigi-logo.ico" alt="" width="64" height="64"/>eigi.ai</a><div><p>HAVE A CHALLENGE IN MIND?</p><a className="email-link" href={contact}>pawanpatil2305@gmail.com <ArrowUpRight size={20}/></a></div><a className="back-top" href="#top" aria-label="Back to top"><ArrowDown size={22}/></a></div><div className="footer-bottom"><span>© {new Date().getFullYear()} eigi_ai</span><span>FORWARD DEPLOYED ENGINEERING</span><span><a href="/credits.txt">3D ASSET CREDITS ↗</a></span></div></footer>
