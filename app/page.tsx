 ...
 return <main id="top"><div className="page-progress" aria-hidden="true"/>
  ...
  <section className="scroll-story" aria-label="From complexity to production">
   <div className={`hero-scene story-sticky ${assembled?'assembled':''}`}>
    <Sculpture assembled={assembled} paused={paused} motion={motion}/>
    <div className="hero-shade"/>
    <p className="scene-eyebrow"><span className="status-dot"/> FORWARD DEPLOYED ENGINEERING</p>
    <div className="story-chapters">
     <div className="story-chapter chapter-opening"><h1>Intelligence,<br/><em>deployed.</em></h1><p>We turn your complex world<br/>into a system that works.</p></div>
    ...
    <div className="hero-bottom"><a href="#experience" className="story-skip">ENTER THE 3D EXPERIENCE <ArrowUpRight size={16}/></a><button className="assemble-button" onClick={()=>setAssembled(!assembled)} aria-pressed={assembled}>{assembled?'RELEASE THE SYSTEM':'CONNECT THE SYSTEM'}<Plus size={18} className={assembled?'rotated':''}/></button></div>
  ...
  <section id="approach" className="intro section-pad">
   ...
   <h2 data-reveal data-scroll-shift="-30">Great AI belongs<br/>in the <span className="blue-text">real world.</span><span className="heading-dot">✳</span></h2>
  ...
  <Sheet open={menu} onOpenChange={setMenu}><SheetContent className="navigation-sheet" showCloseButton={false}><div className="sheet-top"><SheetTitle className="wordmark">eigi_ai</SheetTitle><button className="pill" onClick={()=>setMenu(false)}>CLOSE <X size={18}/></button></div><SheetDescription className="sr-only">Explore eigi_ai forward deployed engineering.</SheetDescription><nav aria-label="Expanded navigation">{[['01','Home','#top'],['02','Our approach','#approach'],['03','Capabilities','#capabilities'],['04','How we work','#method'],['05','The experience','#experience']].map(([n,label,href])=><a href={href} key={n} onClick={()=>setMenu(false)}><span>{n}</span>{label}<ArrowUpRight/></a>)}</nav><a className="sheet-email" href={contact}>Let’s build something useful.<ArrowUpRight/></a></SheetContent></Sheet>
