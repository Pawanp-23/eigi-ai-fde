...
export function KineticText({text,className=''}){
 ...
 return <span ref={ref} className={`kinetic-text ${className}`}>{text.split(' ').map((word,i)=><span className="word-window" key={i}><motion.span initial={reduced?false:{y:'35%',rotate:0}} animate={visible||reduced?{y:'0%',rotate:0}:{y:'35%',rotate:0}} transition={{duration:.85,delay:i*.045,ease:[.16,1,.3,1]}}>{word}</motion.span>{' '}</span>)}</span>;
}
const disciplines=[
...
export function Expertise({paused}){
 ...
 const ribbonProgress=useSpring(scrollYProgress,{stiffness:65,damping:24,mass:.8});
 useEffect(()=>{const el=section.current;if(!el)return;const cards=[...el.querySelectorAll('.expertise-position')];let frame=0,progress=0,target=0,dirty=true,last=performance.now();
  ...
  const render=now=>{const dt=Math.min((now-last)/1000,.05);last=now;frame=requestAnimationFrame(render);if((paused||reduced)&&!dirty&&Math.abs(target-progress)<.0001)return;progress+=(target-progress)*(1-Math.exp(-dt*6));const p=reduced||paused?1:progress,spread=Math.max(0,Math.min(1,(p-.12)/.55)),mobile=innerWidth<700;
   cards.forEach((card,i)=>{const offset=i-1.5;const x=offset*(mobile?86:innerWidth*.235)*spread,y=(mobile?(i%2)*28:Math.abs(offset)*8)*spread+(paused||reduced?0:Math.sin(now*.0008+i*1.5)*22*spread);const rotate=offset*(1-spread)*4 + offset*2*spread+(paused||reduced?0:Math.sin(now*.00065+i)*2.8*spread);card.style.transform=`translate3d(${x}px,${y}px,${i*.5}px) rotate(${rotate}deg)`;card.style.setProperty('--card-turn',`${180*(1-Math.max(0,Math.min(1,(p-.12-i*.045)/.42)))}deg`);});el.style.setProperty('--expertise-drift',`${(1-spread)*8}vw`);el.dataset.spread=spread.toFixed(2);dirty=false;
 ...
 return <section ref={section} id="expertise" className={`expertise ${reduced||paused?'expertise-still':''}`} aria-label="Areas of expertise"><div className="expertise-sticky"><ScrollRibbons progress={ribbonProgress} paused={paused} extended/><div className="expertise-heading"><p className="eyebrow">FROM QUESTION TO PRODUCTION</p><h2><span><KineticText text="Areas of"/></span><span><KineticText text="expertise."/></span></h2><p className="expertise-description">A complete engineering practice.<br/>Built around the work you need to do.</p></div>
