...
export async function createFrontier(host,read,variant){
 ...
 if(portrait){
  ...
  const mat=track(new T.ShaderMaterial({transparent:true,depthWrite:false,blending:T.AdditiveBlending,uniforms:{time:{value:0},tone:{value:new T.Color(0x85d9ff)},spread:{value:0},reveal:{value:1}},vertexShader:'attribute float phase;uniform float time;uniform float spread;uniform float reveal;varying float light;varying float visibility;void main(){vec3 p=position;visibility=1.-smoothstep(reveal*9.-4.5,reveal*9.-3.5,p.y);p.x+=(1.-visibility)*sin(phase*60.)*1.4;float scan=sin(p.y*4.-time*.7);p.x+=sin(phase*80.+time)*spread*.3;p.z+=cos(phase*50.)*spread*.3;vec4 mv=modelViewMatrix*vec4(p,1.);gl_Position=projectionMatrix*mv;gl_PointSize=clamp(18./-mv.z,1.,3.);light=.35+phase*.4+pow(max(scan,0.),18.)*.7;}',fragmentShader:'uniform vec3 tone;varying float light;varying float visibility;void main(){float d=length(gl_PointCoord-.5);if(d>.5)discard;gl_FragColor=vec4(tone*light,(1.-smoothstep(.2,.5,d))*.85*visibility);}'}));
 ...
 function render(now){frame=requestAnimationFrame(render);const dt=Math.min((now-last)/1000,.04);last=now;if(!visible||document.hidden)return;const options=read(),still=options.paused||reduced.matches;const p=reduced.matches?0:options.motion?.current?.progress||0;
  if(still&&!needsRender&&lastProgress===p&&lastDiscipline===options.discipline)return;lastProgress=p;lastDiscipline=options.discipline;needsRender=false;if(!still)time+=dt*.35;const d=1-Math.exp(-dt*6);rx=lerp(rx,still?0:px,d);ry=lerp(ry,still?0:py,d);progress=lerp(progress,p,d);focus=lerp(focus,options.assembled?1:0,d);
  if(portrait){const shift=width<700?0:-1.6;camera.position.set(rx*.35,ry*.2,lerp(15,12.5,clamp(progress/.6,0,1)));camera.lookAt(0,0,0);modelGroup.position.set(shift,-.3,0);modelGroup.rotation.y=rx*.25+Math.sin(time*.15)*.09+(progress-.5)*.28;hologram.material.uniforms.reveal.value=still?1:clamp((progress-.03)/.30,0,1);hologram.material.uniforms.time.value=time;hologram.material.uniforms.spread.value=Math.sin(time*.35)*.3+.35;const tones=[0xa9b9e1,0xa9b9e1,0xa9b9e1];hologram.material.uniforms.tone.value.lerp(new T.Color(tones[options.discipline%3]),d);}
