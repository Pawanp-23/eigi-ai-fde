...
`npm start` previews the built Cloudflare Worker using Wrangler. It does not deploy.

This is a Vinext application with a Cloudflare Worker output, not a conventional
Next.js server or a GitHub Pages export. Keep `.openai/hosting.json` and the Sites
plugin configuration when deploying through the existing Sites project. A GitHub
...
- `archive/media/` preserves superseded videos/posters outside the deployed public directory.
...
Section CSS lives in `app/styles/`; global Tailwind setup stays in `app/globals.css`.
Import order in `app/layout.tsx` is deliberate. Avoid alphabetizing those imports.
Shared UI lives in `components/ui/`; page-specific scenes and sections remain in `app/`.
